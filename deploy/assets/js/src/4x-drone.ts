import { AudioGraph, AudioGraphNode, isOscillatorNode, isGainNode } from './audio-graph.js';
import { DroneGUI, DroneControlCallbacks } from './drone-gui.js';
...
class DroneSynth {
    ...
    oscillators: AudioGraphNode[] = [];
    carrierGains: AudioGraphNode[] = [];
    modGains: Map<string, AudioGraphNode> = new Map(); // keyed by "source_target"
    outputGainNode: AudioGraphNode | null = null;
    clipper: WaveShaperNode | null = null; // store reference to clipper
    connections: AlgorithmConfig = { mod: [], out: [] };
    
    baseFreq: number = 55; 
    freqRatios: number[] = [1.0, 2.0, 3.0, 4.0];
    beatFreqs: number[] = [0.0, 1.5135, 2.3035, 2.46];

    modDepth: number = 1;
    // Operator amplitudes (0-1)
    targetAmps: number[] = [0.25, 0.25, 0.25, 0.25];
    currentAmps: number[] = [0.25, 0.25, 0.25, 0.25];
    targetVolume: number = 0.8;
    currentVolume: number = 0.8;
    animationId: number | undefined = undefined;

    constructor(initialAlgorithm: string) {
    ...
    setModDepth(depth: number) {
        ...
        this.modGains.forEach((modGain) => {
            if (isGainNode(modGain)) {
                ...
                (modGain.webAudioNode as GainNode).gain.value = this.modDepth * (this.baseFreq * this.freqRatios[targetIndex] + this.beatFreqs[targetIndex]);
            }
        });
    }
    
    setBaseFreq(freq: number) {
        this.baseFreq = freq;
        // update all oscillator frequencies
        for (let i = 0; i < this.oscillators.length; i++) {
            if (isOscillatorNode(this.oscillators[i])) {
                (this.oscillators[i].webAudioNode as OscillatorNode).frequency.value = this.baseFreq * this.freqRatios[i] + this.beatFreqs[i];
            }
        }
        ...
        this.modGains.forEach((modGain) => {
            if (isGainNode(modGain)) {
                const match = modGain.name.match(/_(\d+)$/);
                const targetIndex = match ? parseInt(match[1]) : 0;
                (modGain.webAudioNode as GainNode).gain.value = this.modDepth * (this.baseFreq * this.freqRatios[targetIndex]);
            }
        });
    }
    setVolume(vol: number, immediate: boolean = false) {
    ...
    setClipper(clipper: WaveShaperNode) {
    ...
    startSmoothingLoop() {
        const smooth = () => {
            ...
            if (!audioContext || this.carrierGains.length === 0) return;
            ...
            for (let i = 0; i < 4; i++) {
                ...
                if (this.carrierGains[i] && isGainNode(this.carrierGains[i])) {
                    (this.carrierGains[i].webAudioNode as GainNode).gain.setTargetAtTime(
                        cosWeighting(this.currentAmps[i]) * 0.2,
                        scheduledTime,
                        0.015  // small time constant for quick response
                    );
                }
            }
            this.currentVolume += (this.targetVolume - this.currentVolume) * 0.5;
            if (this.outputGainNode && isGainNode(this.outputGainNode)) {
                (this.outputGainNode.webAudioNode as GainNode).gain.setTargetAtTime(
                    this.currentVolume,
                    scheduledTime,
                    0.015
                );
    ...
    start() {
        ...
        const modDepthSlider = document.getElementById('modDepthSlider') as HTMLInputElement;
        const baseFreqSlider = document.getElementById('baseFreqSlider') as HTMLInputElement;
        const volumeSlider = document.getElementById('volumeSlider') as HTMLInputElement;
        
        if (modDepthSlider) this.setModDepth(parseFloat(modDepthSlider.value));
        if (baseFreqSlider) this.setBaseFreq(parseFloat(baseFreqSlider.value));
        if (volumeSlider) this.setVolume(parseFloat(volumeSlider.value), true);
    ...
    updateTargetAmplitudes(normX: number, normY: number) {
...
function createClipper(threshold: number) {
...
const gui = new DroneGUI({
    ...
    onModDepthChange: (depth: number) => {
        droneSynth.setModDepth(depth);
    },
    onBaseFreqChange: (freq: number) => {
        droneSynth.setBaseFreq(freq);
    },
    onVolumeChange: (vol: number) => {
        droneSynth.setVolume(vol);
    },
    onAmplitudeUpdate: (normX: number, normY: number) => {
        droneSynth.updateTargetAmplitudes(normX, normY);
    }
});

function cosWeighting(t: number): number {
